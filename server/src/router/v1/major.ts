...
majorRouter.openapi(
  ...
  async (c) => {
    ...
    const newMajor = await db.delete(majors).where(eq(majors.name, major)).returning();
...
majorRouter.openapi(
  ...
  async (c) => {
    const { name } = c.req.valid('json');
    const newMajor = await db.insert(majors).values({ name }).onConflictDoNothing().returning();
    if (newMajor.length === 0) {
...
majorRouter.openapi(
  ...
  async (c) => {
    ...
    const majorUsers: User[] = await db.select().from(users).where(eq(users.major, majorName));
