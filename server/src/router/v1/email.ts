...
import { createEmailTemplate, getEmailTemplate, updateEmailTemplate, deleteEmailTemplate, listEmailTemplates } from '@/lib/aws/ses';
import { templateMetadataSchema, templateNameSchema, templateSchema } from '@/util/zod';
import { AlreadyExistsException, InvalidTemplateException, LimitExceededException, SESServiceException, TemplateDoesNotExistException } from '@aws-sdk/client-ses';
    ...
    async (c) => {
		try {
			...
			if (error instanceof SESServiceException) {
				return c.json({ error: `AWS SES service error: ${error.message}` }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
			}
			return c.json({ error: `Failed to get email templates: ${error}` }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
		}
    },
);
...
emailRouter.openapi(
	...
	async (c) => {
		try {
			...
			return c.json({ error: `Failed to create email template: ${error}` }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
		}
	},
);
...
emailRouter.openapi(
	...
	async (c) => {
		try {
			...
			return c.json({ error: `Failed to get email template: ${error}` }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
		}
	},
);
...
emailRouter.openapi(
	...
	async (c) => {
		try {
			...
			return c.json({ error: `Failed to update email template: ${error}` }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
		}
	},
);
...
emailRouter.openapi(
	...
	async (c) => {
		try {
			...
			return c.json({ error: `Failed to delete email template: ${error}` }, HttpStatusCodes.INTERNAL_SERVER_ERROR);
		}
	},
);
