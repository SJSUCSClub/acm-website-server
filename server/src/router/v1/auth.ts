...
authRouter.openapi(
  ...
  async (c) => {
    ...
    const url: URL = await googleAuth.createAuthorizationURL(state, codeVerifier, {
...
authRouter.openapi(
  ...
  async (c) => {
    ...
    if (!code || !state || !storedCodeVerifier || state !== storedState) {
      return c.json({ error: 'Missing code, state or code verifier' }, HttpStatusCodes.BAD_REQUEST);
    }
    ...
    try {
      ...
      const { email, name, picture, sub: userId }: GoogleUser = await response.json();
      const existingUser: User = (await db.select().from(users).where(eq(users.email, email)))?.[0];
      let user = existingUser;
      let redirectPath = '/account/dashboard';
      if (!existingUser) {
